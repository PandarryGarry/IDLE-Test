...
**Этапы 1–4 смёржены (PR #6). PR #7 (кинематографическая дорога 0–7 + фиксы iOS + сброс устройства + план адаптива) СМЁРЖЕН в `main` merge-коммитом по команде владельца. Владелец проводит live QA всей дороги в Replit ПОСЛЕ мержа — правки по итогам QA (если что-то не понравится) делает СЛЕДУЮЩАЯ сессия маленькими изолированными коммитами. После QA: слой 1 адаптива (proof-of-concept на auth, `RESPONSIVE_SYSTEM_PLAN.md`) → фундамент 5A «Четырёх Столпов».**
...
- проверить полный маршрут (дорога переработана, см. Сессию 12): cold start → ЗНАК (вывеска) → пролог 6 битов → толчок в auth → rules → ложа/create → УТРО и первый шаг → игра; затем returning user: вывеска 4с → «Дверь узнаёт тебя» → auth → выбор героя → возвращение в рассвет → игра;
