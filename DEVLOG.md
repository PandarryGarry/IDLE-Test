...
**`main` = PR #12** (`591696e`): Этапы 1–4, дорога, WebP, «Экип», манекены.
Replit на `main` ходит в **живую БД**.

**Открытая работа — ветка `arena/01a07240-idle-test`, не смёржена:**
столпы и хаб, Пульт нитей, Путь (шесть дощечек + 12), XP 1→100
(`xpRates.ts`, к тикам не подключено), старт рас 4/3/2/1, тексты `ruleRu`,
вывеска держит первый кадр героя (`lib/bootPreload.ts`). SQL не менялся.

Перед мержем владелец заходит **своим** героем на ветке в Replit.
Следующая большая работа после мержа: привязать уже лежащие иконки
предметов/экипа/UI, потом облако (`save_data`). Не параллелить.
...
**Не сделано:** мерж; привязка 800+ иконок; новая схема Supabase; тики XP.
