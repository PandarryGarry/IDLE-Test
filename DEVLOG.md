...
- `artifacts/my-app/src/pages/HeroHubPage.tsx` + `components/HeroBoard.tsx` —
  экран героя и доска тела (Этап 5B).
...
**Этапы 1–5B в `main` (PR #9, #11 и далее): хотфикс создания героя,
5A (Четыре Столпа), 5B (хаб героя, доска тела, паутина пассивок),
окно «Экип» (манекен, колонки слотов, мини-сумка, 3 набора, сравнение,
бонусы) и арт — 30 манекенов по аватарам (апскейл 1024). Дорога в
Replit проверена владельцем — ок. В текущей ветке `arena/01a06c40`
(сессия 21): ячейки инвентаря и «Экип» приведены к единому плоскому
дереву «каштан» без рамки и без кружка (см. Сессию 21). Ждём фидбек
владельца по цвету ячеек в Replit.**
...
- сначала `DEVLOG.md` (Сессии 18–19), `NEXT_CHAT_HANDOFF.md`, `STAGE5_FOUR_PILLARS_HANDOFF.md`, `scripts/qa/README.md`;
- дорога проверена, «ок» владельца получено; открытая работа — экран героя (5B);
- снимки окна героя без облака: `node scripts/qa/hero.mjs` (канон браузера тот же, `scripts/qa/README.md`);
...
- слой 1 адаптива: шкала токенов `--text-*`/`--space-*`/`--radius-*`/`--control-*` в `:root`, auth целиком на токенах (1 структурный медиа-запрос вместо 3 размерных); слои 2–3 — после вердикта владельца;
- открытые хвосты: удаление неиспользуемого `intro_sign.webp`; подтверждение вида auth после слоя 1;
