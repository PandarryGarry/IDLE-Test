...
**Этапы 1–4 + дорога 0–7 + слой 1 в `main` (PR #6–#8). PR #9 открыт (`arena/01a0526e-idle-test`, НЕ смёржен): hotfix вывески + QA-мок + `tour.mjs` + конвейер WebP-иконок. Владелец на ноутбуке (Replit, ветка PR) сказал: дорога работает; аватарки при создании были медленные — починено WebP + prefetch. Превью Arena не чинить. Мерж — только по слову «мержи». 5A — чат ПОСЛЕ мержа.**

Где продолжать следующему агенту:
- сначала `DEVLOG.md` (Сессия 15–16), `NEXT_CHAT_HANDOFF.md`, `scripts/qa/ACCEPTANCE.md`, `scripts/assets/README.md`;
- **первым делом прогнать `node scripts/qa/tour.mjs` + typecheck, затем ждать «мержи»**;
