...
**`main`:** Этапы 1–5B + раунды 5–6 в коде (хаб, Пульт, Путь, XP-таблица,
`ruleRu`, вывеска греет первый кадр). Одна профессия — «Сбор»; бой на
четырёх столпах. Replit ходит в живую БД. SQL не менялся.

**Текущий статус:** раунды 5–6 смёржены в `main` (PR #15, ветка
`arena/01a07841-idle-test`). Все проверки зелёные:
tsc/build/validate:catalog/test:pillars.

**Следующий чат:** контентный слой поверх чистой модели: привязка иконок
через `iconUrl()`, новые профессии/боевые механики добавляются уже на
столпах, а не на старых «атака/сила/лесорубство». Новый чат сначала
читает документы, изучает архитектуру и **устанавливает QA-браузер**
(`scripts/qa/setup-browser.mjs`), затем ждёт указания владельца.
