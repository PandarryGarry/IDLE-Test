...
Раньше рядом лежал `scripts/in-game-screenshots/` (разовые снимки эмблемы) —
**удалён 2026-09-02**: то же самое и надёжнее делает `scripts/qa`. Знание про
CSS-подмену арта в прогоне не потеряно — оно нужно было только под тот логотип.
...
Ручной эквивалент расписан выше (пакеты, `al2023.tar.br`, шрифты DejaVu).
...
## Окно героя без облака

`hero.mjs` снимает экран героя (Этап 5B) — чтобы править UI, не заходя в Replit:
...
- Сид лежит вне репозитория: `/tmp/aethelia-qa-hero-seed.json`
  (`QA_HERO_SEED`). Это слепок `localStorage.aethelia_qa_db_v1` с героем, у
  которого выкачан луч «Здоровье» и есть вложения в пассивки.
- **Прямой переход на `/hero` не работает**: пока герой не выбран, дорога ведёт
  на выбор персонажа. Скрипт честно проходит: вход → выбор → игра → ссылка
  «Герой» в нижней панели.
- Кадры: `/tmp/aethelia-qa-out/hero` (`QA_HERO_OUT`). Сверх обычных кадров есть
  крупные планы: центр доски (`.hero-board__knot`) и вся доска целиком
  (масштаб ниже игрового — только для просмотра).
- Ссылок на `/hero` две (сайдбар и нижняя панель): на мобилке видна только одна,
  поэтому скрипт кликает по видимой, а не первой.
