...
const RATE_FIELDS: {
  key: 'xpMultiplier' | 'masteryXpMultiplier' | 'actionSpeedMultiplier' | 'dropRateMultiplier' | 'goldMultiplier' | 'sellPriceMultiplier';
  label: string;
  hint: string;
  step: number;
  min: number;
...
const SKILL_TOGGLES: { key: AdminSkillToggle; label: string; icon: string }[] = [
    ...
    <div className="space-y-4 max-w-4xl mx-auto">
      ...
      <section className="rounded-2xl p-4 sm:p-5" style={{ background: 'var(--bg-card)', border: '1px solid #3a2b1a', boxShadow: 'inset 0 2px 8px rgba(0,0,0,0.25)' }}>
        <div className="flex items-center justify-between gap-2 mb-3">
          <div>
            <h2 className="text-sm font-display font-black text-[var(--text-primary)]">⚡ Рейты игры</h2>
            <p className="text-[11px] text-[var(--text-muted)] mt-0.5">Множители скорости, опыта, дропа и цен. Применяются сразу, без перезагрузки.</p>
          </div>
          <button
            type="button"
      ...
      <section className="rounded-2xl p-4 sm:p-5" style={{ background: 'var(--bg-card)', border: '1px solid #3a2b1a', boxShadow: 'inset 0 2px 8px rgba(0,0,0,0.25)' }}>
        <div className="flex items-center justify-between gap-2 mb-3">
          <div>
            <h2 className="text-sm font-display font-black text-[var(--text-primary)]">🧭 Доступность навыков</h2>
            <p className="text-[11px] text-[var(--text-muted)] mt-0.5">Выключенный навык не запустится в игре (активность останавливается и предупреждается).</p>
          </div>
          <button
            type="button"
