...
import { FISHING_SPOTS, FISHING_SPOTS_MAP } from '@/domain/professions/fishing';
      ...
      <div className={`rounded-3xl p-4 sm:p-5 relative overflow-hidden transition-all border ${
        ...
        {isTraining && activeSpot ? (
          ...
          <div className="text-center text-[var(--text-secondary)] flex flex-col items-center gap-2 py-6">
            ...
            <p className="text-xs font-medium" style={{ color: '#e0c070', textShadow: '0 1px 2px rgba(0,0,0,0.5)' }}>{t('fishing.selectSpot')}</p>
          </div>
        )}
      </div>

      <div className="flex items-center justify-between px-1 pt-1">
        <h2 className="text-xs font-extrabold uppercase tracking-widest font-mono flex items-center gap-1.5" style={{ color: '#80f0ff', textShadow: '0 1px 3px rgba(0,0,0,0.6)' }}>
