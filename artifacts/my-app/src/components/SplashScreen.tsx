...
export function SplashScreen({ onLoaded, minDisplayTimeMs = 500 }: SplashScreenProps) {
  ...
  return (
    <div className={`fixed inset-0 z-[999] bg-slate-950 flex flex-col items-center justify-between p-6 select-none transition-opacity duration-300 ${
      ...
      <div className="flex flex-col items-center text-center max-w-sm">
        ...
        <h1 className="font-display font-black text-2xl tracking-widest text-slate-100 mb-1">
          AETHELIA <span className="text-amber-400 font-sans font-extrabold text-lg">IDLE</span>
        </h1>
        <p className="text-xs text-slate-400 font-mono tracking-wider mb-5 flex items-center gap-1.5">
          <Sparkles className="w-3 h-3 text-amber-400" /> Инициализация средневекового мира...
        </p>

        {/* Loading Bar */}
        <div className="w-60 sm:w-72 h-2 bg-slate-900 rounded-full overflow-hidden border border-slate-800 p-0.5 shadow-inner mb-1.5">
          <div 
      ...
      {/* Bottom: Lore Tip */}
      <div className="max-w-md text-center bg-slate-900/70 border border-slate-800/80 rounded-2xl px-4 py-2.5 backdrop-blur-sm">
