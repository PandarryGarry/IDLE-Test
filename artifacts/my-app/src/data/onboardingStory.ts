...
export interface StoryBeat {
  ...
  action?: string;
  /**
   * Атмосфера бита (живой слой над артом):
   * 'dawn' — рассветное дыхание континента: дрейф облачных полос,
   *   пульс восхода, туман в нижней кромке (акт 1 «Континент»);
   * 'road' — пыль дороги: соринки крупнее, ближе и медленнее,
   *   это уже земля под ногами, а не воздух (акт 2 «Тракт»);
   * 'city' — вечерний город живёт: тёплое сияние улицы дышит, из труб
   *   тянутся дымки, вдалеке бродят огоньки (акт 3 «Город»);
   ...
   *   (переход «континент → тракт», свет не обрывается);
   * 'glow' — кадр рождается из разгоревшегося тёплого света
   ...
   * 'ground' — ровнее и ближе к земле, дыхание шага (акт 2 «Тракт»);
   * 'push' — медленный наезд к точке интереса, камера ближе
   *   (акт 3: бит 5, наезд к вывеске «Топора и Пера»).
   */
  motion?: 'air' | 'ground' | 'push';
}
...
export const ENTRANCE_RETURNING: StoryBeat[] = [
  {
    ...
    title: 'Знакомый скрип вывески.',
    body: 'Над головой скрипнула старая вывеска — будто приветствовала. За дверью всё те же огонь и тепло.',
    action: 'Войти в таверну',
    atmosphere: 'threshold',
    finale: true,
  },
