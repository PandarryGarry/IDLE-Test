...
/**
 * Остаток мелворовского наследия: ТОЛЬКО снаряжение (оружие/шлемы/
 * нагрудники/щиты). Всё остальное переехало в каталог (`catalog/`)
 * с русскими именами и картинками, дубли удалены.
 *
 * Снаряжение пока живёт здесь по решению владельца (картинки тиров
 * из `weapons/`/`armor/` привяжем отдельным заходом). Рисуется
 * векторным силуэтом слота (`SquircleSlot` / `getItemVisual`).
 */
