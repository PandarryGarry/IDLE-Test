...
export const BRANCHES: Record<BranchId, NamedBlurb> = {
  ...
  destruction: { id: 'destruction', nameRu: 'Пробой', childRu: 'Пробиваешь чужую защиту.' },
  tempo: { id: 'tempo', nameRu: 'Темп', childRu: 'Делаешь шаги чаще.' },
  evasion: { id: 'evasion', nameRu: 'Уворот', childRu: 'Чаще не попадаешь под удар.' },
  reaction: { id: 'reaction', nameRu: 'Сноровка рук', childRu: 'Быстрее мастеришь и перерабатываешь.' },
...
export const HERO_HELP = {
  ...
  gear: 'Слева две колонки надетого, в центре манекен, справа мини-сумка. Снизу — бонусы от надетого. Наборы сверху: сохранить и надеть. Тап по слоту или вещи открывает карточку. Двуручное занимает обе руки.',
  ...
  path: 'Сверху шесть дощечек: здоровье, удар диапазоном, броня, уворот, крит и удача. Ниже — все двенадцать чисел тела пачками столпов, у каждого столпа свой ур. Диапазон удара растёт вместе с Ударом. Крит пока каркас. Ник, аватар и сброс — в шестерёнке героя.',
} as const;
